...
async def all_views_category(message: Message):    
    ...
    if all_category:
        await message.answer(f'⭐️ Найдено всего {len(all_category)} категорий', reply_markup=all_category_kb())
        await send_many_categories(all_category, bot, message.from_user.id)
    else:
        await message.answer('У вас пока нет ни одной категории!', reply_markup=all_category_kb())