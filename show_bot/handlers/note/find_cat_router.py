...
from keyboards.note_kb import all_category_kb, generate_find_category_keyboard, main_note_kb
...
@find_cat_router.message(F.text == "📝 Заметки по категории")
async def category_views_noti(message: Message, state: FSMContext):
    ...
    if all_category:
        await message.answer(f'Всего категорий: {len(all_category)}', reply_markup=all_category_kb())
        await message.answer('⭐️ Выберете категорию',
...
async def category_noti_process(call: CallbackQuery, state: FSMContext):    
    ...
    if all_notes:
        await call.message.answer(f'⭐️ В категории "{text_name}" заметок: {len(all_notes)}',
                             reply_markup=main_note_kb())
        await send_many_notes(all_notes, bot, call.from_user.id)
    else:
        await call.message.answer(f'Нет ни одной заметки в категории {text_name}!',
                                reply_markup=main_note_kb())    
...
async def text_category_process(message: Message, state: FSMContext):    
    ...
    tar_category = await get_all_categories(user_id=message.from_user.id, text_search=text_search)
    await state.clear()
    if tar_category:
        await message.answer(f'С "{text_search}" в названии категорий: {len(tar_category)}', reply_markup=all_category_kb())      
        await message.answer('⭐️ Найдены категории:',
